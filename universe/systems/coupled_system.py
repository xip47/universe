...
from universe.physics.electromagnetism.coulomb import coulomb_force, coulomb_potential
...
class CoupledSystem:
    ...
    def compute_forces(self) -> List[xp.ndarray]:
        """
        Calcula la fuerza total sobre cada partícula (nuclear + electromagnética).

        Returns
        -------
        List[xp.ndarray]
            Lista de fuerzas sobre cada partícula.
        """
            ...
            for j, pj in enumerate(self.particles):
                ...
                if self.use_coulomb and hasattr(pi.static, 'charge') and hasattr(pj.static, 'charge'):
                    ...
                    if q1 != 0.0 and q2 != 0.0:
                        f_coul = coulomb_force(q1, q2, tuple(xp.asnumpy(pi.position)), tuple(xp.asnumpy(pj.position)))
    ...
    def step(self, dt: float) -> None:
        ...
        for i, p in enumerate(self.particles):
            a = forces[i] / p.static.mass_mev  # Unidades: MeV/c²
            p.velocity += a * dt
            p.position += p.velocity * dt

    def total_energy(self) -> float:
        """
        Calcula la energía total (cinética + nuclear + electromagnética).
        ...
        for i in range(n):
            for j in range(i+1, n):
                ...
                if self.use_coulomb and hasattr(self.particles[i].static, 'charge') and hasattr(self.particles[j].static, 'charge'):
                    ...
                    if q1 != 0.0 and q2 != 0.0:
                        PE_coul += coulomb_potential(q1, q2, tuple(xp.asnumpy(self.particles[i].position)), tuple(xp.asnumpy(self.particles[j].position)))
