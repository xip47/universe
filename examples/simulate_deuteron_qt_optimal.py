"""
Simulación gráfica 2D del deuterón (protón-neutrón) con condición inicial óptima (2.3 fm) y potencial nuclear modular (GeneralNuclearPotential, canal 3S1).

2D graphical simulation of the deuteron (proton-neutron) with optimal initial condition (2.3 fm) and modular nuclear potential (GeneralNuclearPotential, 3S1 channel).

Incluye logs avanzados, reporte de energía de enlace y visualización profesional con PyQtGraph.

Usage
-----
Ejecuta este script para visualizar el estado ligado realista del deuterón y comparar con la energía de enlace experimental.
...
dt: float = 0.005  # fm/c (mayor precisión)
steps: int = 5000
sep_opt: float = 2.3  # fm (condición óptima encontrada)

# --- Inicialización de protón y neutrón ---
pos_init: List[backend.xp.ndarray] = [backend.xp.array([0.0, 0.0]), backend.xp.array([sep_opt, 0.0])]
...
app = QApplication([])
win = pg.GraphicsLayoutWidget(show=True, title="Simulación 2D: Deuterón óptimo (GeneralNuclearPotential 3S1)")
win.resize(1200, 700)
